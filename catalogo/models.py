...
class Produto(models.Model):
    ...
    group = models.ForeignKey('catalogo.GroupProduct',
    ...
    tipo = models.PositiveIntegerField(
        'Tipo', default=PRODUTO, choices=TIPO_LIST)
    valor = models.DecimalField('Valor', max_digits=10, decimal_places=2)
    obs = models.TextField('Observação', null=True, blank=True)
    quantidade = models.IntegerField(
        'Quantidade', default=0, null=True, blank=True)
    data_create = models.DateTimeField(
        'Criado', auto_now_add=True, auto_now=False)
    data_update = models.DateTimeField(
        'Alterado', auto_now_add=False, auto_now=True)
